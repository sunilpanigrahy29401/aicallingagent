...
async def get_all_settings() -> dict:
    ...
    for row in (result.data or []):
        ...
        if k == "TEST_KEY":
            continue
        # Skip overwriting if the environment variable is already providing a value
        if os.getenv(k):
            continue
        if k in SENSITIVE_KEYS:
...
async def get_stats() -> dict:
    ...
    return {"total_calls": total_calls, "booked": booked, "not_interested": not_interested, "avg_duration_seconds": round(avg_dur, 1), "booking_rate_percent": booking_rate, "outcomes": outcomes, "timeline": timeline, "duration_by_outcome": duration_by_outcome}
